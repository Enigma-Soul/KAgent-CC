...
每次提交涉及功能变更或修复时，**必须同步更新版本号和 CHANGELOG**：

1. `package.json`（根）和 `extension/package.json` 的 `version` 字段必须一致
2. `CHANGELOG.md` 顶部新增 `# x.y.z` 段落，格式参照已有条目（`### Feat/Fix/Chore(模块)` + 列表）
3. CI auto-tag 读取 `extension/package.json` 的版本号打 tag，npm publish 读 `package.json` 的版本号发布
4. **版本号不能重复** -- npm 不允许覆盖已发布的版本，否则 CI 会报 `E403 You cannot publish over the previously published versions`
5. auto-tag 会在 PR 合并 main 时自动检测：如果 tag 已存在则 bump patch，但手动提交时也应提前改好版本号
...
- `bin/kagent-cc.mjs` - CLI 入口，dispatch install/uninstall/tui（动态 import 用 `pathToFileURL` 兼容 Windows）
...
- `ci.yml` - push/PR 到 main/develop 时：Bun 打包 dist/ + 编译扩展；PR 仅编译检查不打包，push 时才打包上传 artifact
- `release.yml` - tag `v*` 触发：npm publish + Open VSX publish + GitHub Release（body 从 CHANGELOG.md 提取）；PR 合并 main 自动 bump patch + 打 tag
- auto-tag 只推 tag 不推分支（main 有分支保护，只接 PR）
- 需要 GitHub Secrets：`NPM_TOKEN`（Automation 类型，不要求 OTP）、`OVSX_PAT`
...
- `.claude/`、`.vscode/`、`.cursor/`、`demo/` 已 gitignore，不入库
- K 线配色：cn=红涨绿跌，us=绿涨红跌，可切换
- 每次功能/修复提交必须同步改版本号 + CHANGELOG.md
